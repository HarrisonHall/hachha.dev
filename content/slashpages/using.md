# {{icon wrench}} Defaults

These days, I tend to favor [mise](https://mise.jdx.dev/) for cli tooling, and
[nix](https://nixos.org/) for my dev box and other graphical applications. My
[dotfiles](https://github.com/HarrisonHall/dotfiles) are available.

Check my [self-hosted](/self-hosted) services for additional software.

## System Utilities

I use Linux casually and professionally. I typically prefer lightweight tiling
window managers over full-blown desktop environments. With the little PC gaming
I do, I haven't had surprisingly few issues using with Steam on Linux.

| Utility       | Tool                        |
| ------------- | --------------------------- |
| OS            | NixOS, Alpine               |
| WM            | Niri, Cosmic                |
| Terminal      | Wezterm, Ghostty            |
| `$SHELL`      | Fish                        |
| `$EDITOR`     | Helix                       |
| `$VISUAL`     | Kate                        |
| VCS           | `jj` (`git`)                |
| Forge         | `radicle`, Github           |
| Feeds         | `slipstream`, Feeder        |
| Tasks         | `cfait`                     |
| Comms         | Discord, Signal, ProtonMail |
| Browser       | Firefox                     |
| Widgets       | rofi                        |
| Notifications | Mako                        |

## Languages

I reach for rust on most personal projects since it has an expressive type
system, convenient build/packaging system, and high-quality ecosystem. I
typically treat less as a systems programming language and more as a high-level
language.

| Language   | Usage                                           |
| ---------- | ----------------------------------------------- |
| Rust       | Most personal projects, systems programming     |
| Python     | Scripting, general development (professionally) |
| TypeScript | Web development (professionally)                |
| Zig        | Systems programming                             |
| C/C++      | Systems programming (professionally)            |

## Misc. Tooling

### The Essentials

| Tool    | Utility                          |
| ------- | -------------------------------- |
| tmux    | Terminal multiplexing.           |
| bat     | cat with color (wings?).         |
| bottom  | Clean top.                       |
| eza     | Fancy ls.                        |
| ripgrep | Just good search.                |
| yazi    | Terminal navigation. See zoxide. |

### The Newcomers

| Tool     | Utility                 |
| -------- | ----------------------- |
| atuin    | Shell history handling. |
| starship | Just a good prompt.     |
| zoxide   | cd with a brain.        |

### Graphical Essentials

| Tool      | Utility                |
| --------- | ---------------------- |
| Anki      | Japanese SRS.          |
| Godot     | Game engine.           |
| LocalSend | Share files w/o cords. |
| fcitx5    | Japanese IME (mozc).   |

## Watching

There are existing tools that I haven't quite yet incorporated into my
workflows. With most of these, I'm waiting until some key feature is implemented
before I switch over.

| Tool   | Reason                                                             |
| ------ | ------------------------------------------------------------------ |
| fnox   | Need a way to share secrets.                                       |
| zellij | Love tmux, wouldn't mind something more modern.                    |
| yolk   | I'm shocked there's no "correct" way to handle templating in 2026. |
